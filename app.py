...
# your_itinerary     
@app.route('/your_itinerary')
def your_itinerary():
    customer_exists = session.get('customer_exists')
    cust_id = customer_exists[0]
    if customer_exists:
        cursor = mysql.connection.cursor()
        cursor.execute('''SELECT * FROM `itinerary` INNER JOIN `token` ON token.token_id = itinerary.token_id WHERE token.cust_id = %s ORDER BY itinerary.token_id DESC;''',(cust_id,))
        itinerary_printing = cursor.fetchall()
        print('itinerary_printing',itinerary_printing)
        cursor.execute('''SELECT * FROM `payments` INNER JOIN booking ON booking.booking_id = payments.booking_id INNER JOIN customer ON customer.cust_id = payments.cust_id INNER JOIN token ON token.token_id = booking.token_id WHERE customer.cust_id = %s ORDER BY payments.payment_id DESC;''',(cust_id,))
        price_printing = cursor.fetchall()
        print('price_printing',price_printing)
        cursor.execute('''SELECT * FROM `booking` INNER JOIN token ON token.token_id = booking.token_id WHERE token.cust_id = %s ORDER BY booking.booking_id DESC;''',(cust_id,))
        booking_printing = cursor.fetchall()
        print('booking_printing',booking_printing)
        
        return render_template('/Travelbuddycustomer/your_itinerary.html', itinerary_printing=itinerary_printing, price_printing=price_printing,booking_printing=booking_printing)
...
@app.route('/update_docstore', methods=['POST'])
def update_docstore():
    customer_exists = session.get('customer_exists')
    cust_id = customer_exists[0]
    ...
    cursor = mysql.connection.cursor()
    cursor.execute('''SELECT * FROM `docs` WHERE `cust_id` = %s;''',(cust_id,))
    docs_data =cursor.fetchall()
    if not docs_data:
        cursor.execute('''INSERT INTO `docs` (`doc_id`, `cust_id`, `aadhar`, `Driving_license`, `passport`) VALUES (NULL, %s, %s, NULL , %s);''',(cust_id,aadhar,passport,))
    else:
        cursor.execute('''UPDATE docs SET `aadhar` = %s, `passport`= %s WHERE `cust_id` = %s;''',(aadhar,passport,cust_id))
...
def payment_data():
    customer_exists = session.get('customer_exists')
    if customer_exists:
        insert_kor_data_into_the_database = inserting()
        print(insert_kor_data_into_the_database)
        print("------------------------------------")
        cust_id = customer_exists[0]
        ...
        print(upi)
        print(cust_id)
        cursor.execute('''INSERT INTO `payments` (`payment_id`, `cust_id`, `upi_id`, `booking_id`) VALUES (NULL, %s, %s, %s);''',(cust_id,upi,booking_id[0][0]))
        mysql.connection.commit()
...
def inserting():
    ...
    if customer_exists:
        ...
        cursor.execute('''INSERT INTO `booking` (`booking_id`, `token_id`, `travel_type`, `trip_type`, `arrival_date`, `departure_date`, `source`, `destination`, `numTravelers`, `ageGroup`) VALUES (NULL, %s, %s, %s, %s, %s, %s, %s, %s, %s);''',(token_id,travel_type,trip_type,start,end,org,dest,nots,age_range))
        mysql.connection.commit()
        # booking=cursor.fetchall()
...
def submit():
    ...
    if creating_days:
        ...
        print('customer  ',customer_exists) 
    ...
    return render_template('/Travelbuddycustomer/display_itenary.html', org=org, dest=dest,start=start,end=end, budget=budget,days=days,transportation=transportation, hotel=hotel_global, creating_days=creating_days,total_cost=total_cost) ## --TO PRINT DATA BACK TO HTML
