...
def get_train(source, destination):
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM trains WHERE depart_station=%s AND arrive_station=%s ORDER BY RAND() LIMIT 1", (source, destination))
...
def get_bus(source, destination):
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM bus WHERE depart_stand=%s AND arrive_stand=%s ORDER BY RAND() LIMIT 1", (source, destination))
    ...
    # first check for flight than train and last bus
        for flight in flight_avaliable:
            if flight[8]*no_of_travlers <= percentage:
                ...
                # print(budget)
                transportation_name = 'flight'
                return budget,flight,transportation_name
            else:
                train_avaliable = get_train(source, destination)
                for train in train_avaliable:
                    if train[10]*no_of_travlers<=percentage:
                        ...
                        budget = (train[10]*no_of_travlers)
                        # print(budget)
                        transportation_name = 'train'
                        return budget,train,transportation_name
                    else:
                        bus_avaliable = get_bus(source, destination)
                        for bus in bus_avaliable:
                            if bus[9]*no_of_travlers<=percentage:
                                print("Avaliable Bus: ")
                                print(f"Bus Number: {bus[1]}, Source: {bus[2]}, Destination: {bus[3]}, Departure Time: {bus[4]}, Arrival Time: {bus[5]}, Price: {bus[6]}")
                                budget = (bus[9]*no_of_travlers)
                                # print(budget)
                                transportation_name = 'bus'
                                return budget,bus,transportation_name
                            else:
                                print("Sorry, Your budget is too low. Please Increase your budget")
    
    elif not flight_avaliable:
        train_avaliable = get_train(source, destination)
        for train in train_avaliable:
            if train[10]*no_of_travlers<=percentage:
                print("Avaliable Train: ")
